...
mcmc_list <- lapply(1:dim(theta_samples)[2], function(i) mcmc(theta_samples[, i]))
stan_mcmc_list <- mcmc.list(mcmc_list)

# MCMC diagnostics
gelman_diag <- gelman.diag(stan_mcmc_list)
effective_size <- effectiveSize(stan_mcmc_list)
ess_table <- data.frame(Gene = 1:n_genes, EffectiveSize = as.numeric(effective_size))
...
density_plots_list <- lapply(1:n_genes, function(i) {
  ggplot(data.frame(x = theta_samples[, i]), aes(x = x)) +
